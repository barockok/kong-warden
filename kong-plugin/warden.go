...
import (
	...
	"github.com/barockok/kong-warden/permit"
...
func ForwadEffect(kong *pdk.PDK, permission WardenPermission) {
	...
	for _, s := range permission.Selector {
		if strings.HasPrefix(s, permit.EFFECT_FORWARD_ALLOW) {
			headerVal = append(headerVal, fmt.Sprintf("%s%s", permit.EFFECT_ALLOW, strings.TrimPrefix(s, permit.EFFECT_FORWARD_ALLOW)))
		}
		if strings.HasPrefix(s, permit.EFFECT_FORWARD_DENY) {
			headerVal = append(headerVal, fmt.Sprintf("%s%s", permit.EFFECT_DENNY, strings.TrimPrefix(s, permit.EFFECT_FORWARD_DENY)))
		}
	}
...
func EvaluateSelector(kong *pdk.PDK, permission WardenPermission) bool {
	...
	if slices.Index([]string{"post", "put", "patch"}, strings.ToLower(httpMethod)) > -1 && contentType == "application/json" {
		...
		return permit.AttributeMatch(payload, permission.Selector)
	}
