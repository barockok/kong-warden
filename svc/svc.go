...
func wardenMiddleware(next echo.HandlerFunc) echo.HandlerFunc {
	return func(c echo.Context) error {
		...
		c.Set(WardenPermissionEchoKey, permit.NewPrimitiveQuery(permissions))
		return next(c)
	}
}
func AuthorizeResource(c echo.Context, i interface{}) error {
	permissions := c.Get(WardenPermissionEchoKey).(*permit.PrimitiveQuery)
...
func StartSVC() {
	...
	e.GET("/", func(c echo.Context) error {
		...
		permission := c.Get(WardenPermissionEchoKey).(*permit.PrimitiveQuery)
